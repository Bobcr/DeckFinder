    ...
    struct Card: Codable, EmptyInitializable {
        ...
        func populate() -> Self {
            return .init(name: self.name,
                         ...
                         enumValue: .find(by: .id, value: self.id),
...
extension PlayerProfile {
    struct ProfileCard: Codable, EmptyInitializable {
        ...
        func populate() -> Self {
            typealias Card = Funcs.Card

            let enumValue: DataSet.Cards = .find(by: .id, value: self.id)
...
extension PlayerProfile: DecodeConvertible {
    func actionsToBeTakenAfterCompletion(datas: DatasEnvObj, arg: String) {
        ...
        let sortMode = PlayerBaseView.CardsView.getSortMode()
        let orderMode = PlayerBaseView.CardsView.getOrderMode()
        populated.cards = PlayerBaseView.CardsView.sortPlayerCards(cards: populated.cards,
                                                                   sort: sortMode,
                                                                   order: orderMode)
        GeneralModels.Players.SearchedPlayers
            .add(searchedPlayer: .init(name: populated.name,
                                       tag: populated.tag,
                                       clanName: populated.clan.name,
                                       clanTag: populated.clan.tag,
                                       clanBadgeName: populated.clan.badgeName,
                                       searchCount: 0))
        
        datas.playerProfile.set(to: populated)
    }
}

extension PlayerProfile {
    func populate() -> OAPI.Models.PlayerProfile {
